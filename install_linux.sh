...
get_latest_release() {
  wget -O - -q "https://api.github.com/repos/terraform-linters/tflint/releases/latest" | # Get latest release from GitHub api
...
wget -q -O "${download_zip}" "https://github.com/terraform-linters/tflint/releases/download/${version}/tflint_${os}.zip"
retVal=$?
if [ $retVal -ne 0 ]; then
...
unzip "${download_zip}" -d "${download_path}"
if [[ $os == "windows"* ]]; then
